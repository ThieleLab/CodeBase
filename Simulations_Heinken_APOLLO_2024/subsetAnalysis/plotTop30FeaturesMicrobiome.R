...
subsets <- c("Adults_body_sites_healthy","Adults_healthy_by_country","Adults_vs_infants_healthy","IBD_vs_healthy","Infants_premature_vs_healthy","Infants_undernourished_vs_healthy","Infection_antibiotics_vs_no_antibiotics","Infection_resistant_vs_susceptible","Infection_vs_healthy","Obesity_vs_normalweight","PD_vs_healthy","T2D_vs_healthy")

#datasets <- c("Reaction_abundance_","Reactions_presence_","Subsystem_abundance_")
datasets <- c("Reaction_abundance_")

for (i in 1:length(subsets)) {
  infoPath = paste("Analysis_microbiome_models/Subgroup_analysis/Subgroups/",subsets[i],"/",subsets[i],"_samples.csv",sep="")
  if (i==1) {
  ...
  for (j in 1:length(datasets)) {
      ...
      dataFile = paste("Analysis_microbiome_models/Summary_for_figures/Feature_heatmaps/",datasets[j],subsets[i],".csv",sep="")
      ...
      imagePath =  paste("Heatmaps/Microbiomes/TopFeatures_",datasets[j],subsets[i],".png",sep="")
      if (nrow(x) > 51) {
